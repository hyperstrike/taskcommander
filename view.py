...
class Application(Frame):
    ...
    def __init__(self,master=None):
        ...
        myframe1.grid(row=2,column=0,rowspan=2,columnspan=2,sticky=ALL)
        ...
        arrowLeft = Button(tabFrame,text="<", command=self.GoLeft).grid(column=lowBounds-1, row=1)
        arrowRight = Button(tabFrame,text=">", command=self.GoRight).grid(column=highBounds, row=1)

        
        test = Button(tabFrame,text="Open", command=self.OnButtonClick)
        myButtons = [test] * 50


        for x in range (lowBounds, highBounds):
            myButtons[x] = Button(tabFrame,text="Project" + str(x), command=self.OnButtonClick).grid(column=x, row=1)
        ...
        myLabels = Label(myframe2, text="Task " + str(x)).grid(column=0, row=0)
        ...
        ############ 	 INFO FRAME	    ############
        ############################################
        info_ownerName = "info_ownerName"
        info_creatorName = "info_creatorName"
        info_dateOfCreation = "info_dateOfCreation"
        info_dueDate = "info_dueDate"
        info_parentProject = "info_parentProject"
        self.rowconfigure(0)
        self.columnconfigure(2)
        titleFrame=Frame(self,bg='red',width=400)
        titleFrame.grid(row=0,column=3,rowspan=1,columnspan=3,sticky=ALL)
        Label(titleFrame, text="*Insert Project/Task Name*").grid(row=2, column=1)
        Label(titleFrame, text="*Insert Progress Bar Here*").grid(row=4, column=1)
        myframe3=Frame(self,bg='grey',width=400)
        myframe3.grid(row=2,column=3,rowspan=2,columnspan=3,sticky=ALL)
        ...
        Label(myframe3, text=info_ownerName).grid(row=3, column=2)
        Label(myframe3, text=info_creatorName).grid(row=4, column=2)
        Label(myframe3, text=info_dateOfCreation).grid(row=5, column=2)
        Label(myframe3, text=info_dueDate).grid(row=6, column=2)
        Label(myframe3, text=info_parentProject).grid(row=7, column=2)


    def OnButtonClick(delf):
        1+1

    def GoRight(self):
        global lowBounds
        global highBounds
        lowBounds = lowBounds + 1
        highBounds = highBounds + 1
